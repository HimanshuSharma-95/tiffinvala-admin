...
export default function EditItemPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const formData = {
                ...
                variants: p.variants.map(v => ({ size: v.size, price: String(v.price) })),
            }
            setForm(formData)
    ...
    const handleRemoveCategory = async (id: string) => {
        ...
        try {
            ...
            if (
                removedCategory &&
                form.category === removedCategory.name
            ) {
                setForm(p => ({
                    ...p,
                    category: '',
    ...
    const addVariant = () => setForm(p => ({ ...p, variants: [...p.variants, { size: '', price: '' }] }))
    ...
    const handleCategorySelect = (cat: string) => {
        setForm(p => ({ ...p, category: cat, food_class: cat, product_type: cat }))
    }

    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!form.name || !form.category) { toast.error('Name and category are required'); return }
        if (form.variants.some(v => !v.size || !v.price)) { toast.error('Fill all variant fields'); return }
                ...
                <form onSubmit={handleSave} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                            placeholder="Dal Makhani" className="w-full mt-1 bg-gray-100 rounded-xl px-4 py-3 text-sm outline-none" />
                    </div>
                    ...
                    <div>
                        ...
                        {showCategoryManager && (
                            <div className="bg-gray-50 rounded-2xl p-3 mb-3 space-y-3">
                                <p className="text-xs font-semibold text-[#1E2A3A]">Manage Categories</p>
                                <div className="flex gap-2">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 mt-2">
                                {product.areas.map(area => (
                                    <span key={area} className="text-xs bg-green-100 text-green-600 px-3 py-1 rounded-full font-medium capitalize">
                                        {area.replace('_', ' ')}
                                    </span>
                                ))}
                            </div>
