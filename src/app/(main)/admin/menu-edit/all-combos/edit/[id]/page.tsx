...
import {
    getSingleCombo, getCategories, updateCombo,
    updateComboImage, removeComboImage,
    deleteCombo
...
export default function EditComboPage() {
    ...
    const isFormChanged = () => {
        ...
        return JSON.stringify({ ...form, rules: JSON.stringify(rules) }) !== JSON.stringify(initialForm)
    }
    ...
    const handleImageUpload = async () => {
        ...
        try {
            ...
            setCombo(prev => prev ? { ...prev, image: res.data?.image || '' } : prev)
            setNewImage(null)
            setPreviewUrl(null)
    ...
    const addRule = () => setRules(p => [...p, { category: [], quantity: 1, label: '', allowCustomSelection: true }])
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        if (rules.some(r => r.category.length === 0)) { toast.error('Each rule needs at least one category'); return }
            ...
            <div className="max-w-md mx-auto px-4 space-y-5">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex justify-center">
                        <div className="w-48 h-48 rounded-2xl overflow-hidden bg-gray-100 relative">
                            ...
                            <img
                                ...
                                className="w-full h-full object-cover"
                                onError={(e) => {
                                    (e.currentTarget as HTMLImageElement).src = '/defaultfood.png'
                                }}
                            />
                    ...
                    <div className="flex gap-2">
                        ...
                        {newImage && (
                            <button type="button" onClick={() => { setNewImage(null); setPreviewUrl(null) }}
                ...
                <form onSubmit={handleSave} className="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
                            rows={2} className="w-full mt-1 bg-gray-100 rounded-xl px-4 py-3 text-sm outline-none resize-none" />
                    </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-xs text-gray-500 font-medium">COMBO RULES</label>
                        ...
                        <div className="space-y-3">
                            {rules.map((rule, index) => (
                                <div key={index} className="bg-gray-50 rounded-2xl p-3 space-y-3">
                                    <div className="flex items-center justify-between">
                                        <p className="text-xs font-semibold text-[#1E2A3A]">Rule {index + 1}</p>
                                        {rules.length > 1 && (
                                            <button type="button" onClick={() => removeRule(index)} className="text-red-400">
                                                <X size={14} />
                                            </button>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="number" min={1} value={rule.quantity}
                                    ...
                                    <div>
                                        <label className="text-xs text-gray-400 block mb-1.5">
                                            ...
                                            {rule.category.length > 0 && (
                                                <span className="text-[#F97316] ml-1">({rule.category.join(', ')})</span>
                                            )}
                                        </label>
                                            ...
                                            {categories.map(cat => (
                                                <button key={cat._id} type="button"
                                                    ...
                                                    className={`px-2.5 py-1 rounded-full text-xs font-medium border capitalize transition-colors
                                                        ${rule.category.includes(cat.name)
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button type="button"
                                            onClick={() => updateRule(index, 'allowCustomSelection', !rule.allowCustomSelection)}
                                            className={`w-10 h-5 rounded-full transition-colors ${rule.allowCustomSelection ? 'bg-[#F97316]' : 'bg-gray-300'}`}>
                                            <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform mx-0.5
                                                ${rule.allowCustomSelection ? 'translate-x-5' : 'translate-x-0'}`} />
                                        </button>
                                        <span className="text-xs text-gray-500">Custom selection allowed</span>
                                    </div>
                                </div>
                            ))}
                        </div>
